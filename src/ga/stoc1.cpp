...
double LnFac(int32_t n) {
   ...
   if (n < FAK_LEN) {
      if (n <= 1) {
         if (n < 0) throw GAError("Parameter negative in LnFac function");  
         return 0;
      }
...
int32_t StochasticLib1::Hypergeometric (int32_t n, int32_t m, int32_t N) {
   ...
   */   
   ...
   if (n > N || m > N || n < 0 || m < 0) {
      throw GAError("Parameter out of range in hypergeometric function");}
...
void StochasticLib1::MultiHypergeometric (int32_t * destination, int32_t * source, int32_t n, int colors) {
   ...
   */
   int32_t sum, x, y;
   int i;
   if (n < 0 || colors < 0) throw GAError("Parameter negative in multihypergeo function");
   if (colors == 0) return;

   // compute total number of balls
   for (i=0, sum=0; i<colors; i++) { 
      y = source[i];
      if (y < 0) throw GAError("Parameter negative in multihypergeo function");
      sum += y;
   }
   if (n > sum) throw GAError("n > sum in multihypergeo function");
...
int32_t StochasticLib1::Poisson (double L) {
   ...
   */
   ...
   if (L < 17) {
      if (L < 1.E-6) {
         if (L == 0) return 0;
         if (L < 0) throw GAError("Parameter negative in poisson function");
      ...
      if (L > 2.E9) throw GAError("Parameter too big in poisson function");
...
int32_t StochasticLib1::Binomial (int32_t n, double p) {
   ...
   */
   ...
   if (n <= 0 || p <= 0) {
      ...
      // error exit
      throw GAError("Parameter out of range in binomial function"); 
   }
...
void StochasticLib1::Multinomial (int32_t * destination, double * source, int32_t n, int colors) {
   ...
   */
   ...
   int32_t x;
   int i;
   if (n < 0 || colors < 0) throw GAError("Parameter negative in multinomial function");
   if (colors == 0) return;

   // compute sum of probabilities
   for (i=0, sum=0; i<colors; i++) { 
      s = source[i];
      if (s < 0) throw GAError("Parameter negative in multinomial function");
      sum += s;
   }
   if (sum == 0 && n > 0) throw GAError("Zero sum in multinomial function");
...
void StochasticLib1::Multinomial (int32_t * destination, int32_t * source, int32_t n, int colors) {
   ...
   for (i=0, sum=0; i<colors; i++) { 
      p = source[i];
      if (p < 0) throw GAError("Parameter negative in multinomial function");
      sum += p;
   }
   if (sum == 0 && n > 0) throw GAError("Zero sum in multinomial function");
...
double StochasticLib1::NormalTrunc(double m, double s, double limit) {
   ...
   if (limit < s) throw GAError("limit out of range in NormalTrunc function");
   double x;
   do {
      x = Normal(0., s);
...
int StochasticLib1::Bernoulli(double p) {
   ...
   if (p < 0 || p > 1) throw GAError("Parameter out of range in Bernoulli function");
   return Random() < p;
}
