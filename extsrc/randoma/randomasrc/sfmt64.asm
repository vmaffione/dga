...
L2:     mov     [par1+rbx*4].CRandomSFMTA.STATE,eax  ; initialize state
        cmp     ebx, SFMT_N*4 - 1
        jb      L1
...
InitAndPeriod   proc    private
        push    rbx
        
        ; initialize constants for Mother-Of-All
        movaps  xmm0, xmmword ptr [InitMother]
        movaps  xmmword ptr [par1].CRandomSFMTA.MF3, xmm0
        movaps  xmm0, xmmword ptr [InitMother+16]
        movaps  xmmword ptr [par1].CRandomSFMTA.MF1, xmm0
        
        ; initialize constants for SFMT
        movaps  xmm0, xmmword ptr [InitMask]
        movaps  xmmword ptr [par1].CRandomSFMTA.AMASK, xmm0

        ; initialize various variables
        xor     eax, eax
        mov     dword ptr [par1].CRandomSFMTA.one, eax
        mov     dword ptr [par1+4].CRandomSFMTA.one, 3FF00000H
        mov     dword ptr [par1].CRandomSFMTA.LASTINTERVAL, eax        
        ...
        movaps  xmm1, xmmword ptr [par1].CRandomSFMTA.STATE
        andps   xmm1, xmmword ptr [InitParity]
        ...
        lea     rdx, [InitParity]
L5:     mov     eax, [rdx+rbx*4]       ; InitParity[i]
        test    eax, eax
        jnz     L6
        inc     ebx
...
L8:     cmp     [par1].CRandomSFMTA.USEMOTHER, 0
        je      L9
...
SFMT_MID equ ((SFMT_SIZE - SFMT_LAG) / 2)
        ...
        xor     eax, eax
        cmp     par5d, eax             ; IncludeMother
        setnz   al                     ; convert any nonzero value to 1

        push    rbx
        push    rbp
        
        cmp     par2d, (type CRandomSFMTA); ThisSize
        jb      Error                  ; Error exit if buffer too small
...
L1:     ; r = sta[i] ^ sta[(i + mid) % size] ^ sta[(i + size - 1) % size];
        ...
        jb      L2
        sub     ebp,  SFMT_SIZE
L2:     xor     eax,  [par1+rbp*4].CRandomSFMTA.STATE   ; sta[(i + mid) % size]
        lea     r10d, [rbx+SFMT_SIZE-1]
        cmp     r10d, SFMT_SIZE
        jb      L3
        sub     r10d, SFMT_SIZE
L3:     xor     eax,  [par1+r10*4].CRandomSFMTA.STATE   ; sta[(i + size - 1) % size]
...
L4:
        ...
        jb      L5
        sub     ebp, SFMT_SIZE
L5:     add     [par1+rbp*4].CRandomSFMTA.STATE, eax
...
L10:    ; r = sta[i] + sta[(i + mid) % size] + sta[(i + size - 1) % size]
        ...
        jb      L11
        sub     ebp,  SFMT_SIZE
L11:    add     eax,  [par1+rbp*4].CRandomSFMTA.STATE   ; sta[(i + mid) % size]
        lea     r10d, [rbx+SFMT_SIZE-1]
        cmp     r10d, SFMT_SIZE
        jb      L12
        sub     r10d, SFMT_SIZE
L12:    add     eax,  [par1+r10*4].CRandomSFMTA.STATE    ; sta[(i + size - 1) % size]
        ...
        jb      L13
        sub     ebp, SFMT_SIZE
L13:    xor     [par1+rbp*4].CRandomSFMTA.STATE, eax
...
L14:
        ...
        ; if (UseMother) {
        cmp     [par1].CRandomSFMTA.USEMOTHER, 0
        jz      L20
...
;assume  par1: ptr CRandomSFMTA
        ...
        movdqa  xmm1, xmmword ptr [par1].CRandomSFMTA.M3  ; load M3,M2
        movdqa  xmm2, xmmword ptr [par1].CRandomSFMTA.M1  ; load M1,M0
        movhps  qword ptr [par1].CRandomSFMTA.M3, xmm1    ; M3=M2
        movq    qword ptr [par1].CRandomSFMTA.M2, xmm2    ; M2=M1
        movhps  qword ptr [par1].CRandomSFMTA.M1, xmm2    ; M1=M0
        pmuludq xmm1, xmmword ptr [par1].CRandomSFMTA.MF3 ; M3*MF3, M2*MF2
        pmuludq xmm2, xmmword ptr [par1].CRandomSFMTA.MF1 ; M1*MF1, M0*MF0
        paddq   xmm1, xmm2                               ; P3+P1, P2+P0
        movhlps xmm2, xmm1                               ; Get high qword
        movq    xmm3, qword ptr [par1].CRandomSFMTA.MC    ; +carry
        paddq   xmm1, xmm3
        paddq   xmm1, xmm2                               ; P0+P1+P2+P3
        movq    qword ptr [par1].CRandomSFMTA.M0, xmm1    ; Store new M0 and carry
        ret
Mother_Next endp


align 16
SFMT_Generate proc private
...
;assume  par1: ptr CRandomSFMTA
        ...
        movdqa  xmm1, xmmword ptr [par1+(SFMT_N-2)*16].CRandomSFMTA.STATE
        movdqa  xmm2, xmmword ptr [par1+(SFMT_N-1)*16].CRandomSFMTA.STATE
...
align 8
L1:     movdqa   xmm0, xmmword ptr [par1+rdx+SFMT_M*16]       ; b
        psrld    xmm0, SFMT_SR1                               ; b1
        pand     xmm0, xmmword ptr [par1].CRandomSFMTA.AMASK  ; b1
        movdqa   xmm3, xmmword ptr [par1+rdx]                 ; a
        pxor     xmm0, xmm3
        ...
        pxor     xmm2, xmm0                                   ; r2 = r
        ; state[i] = r;
        movdqa   xmmword ptr [par1+rdx], xmm2
...
L2:     ; second i loop from SFMT_N - SFMT_M + 1 to SFMT_N
        movdqa   xmm0, xmmword ptr [par1+rdx+(SFMT_M-SFMT_N)*16]; b
        psrld    xmm0, SFMT_SR1                               ; b1
        pand     xmm0, xmmword ptr [par1].CRandomSFMTA.AMASK  ; b1
        movdqa   xmm3, xmmword ptr [par1+rdx]                 ; a
        pxor     xmm0, xmm3
...
L8:     cmp     [par1].CRandomSFMTA.AMASK, SFMT_MASK1
        jne     Error                  ; Make error if not initialized

        ; ix = 0;
        mov      [par1].CRandomSFMTA.IX, 0 ; point to start of STATE buffer
        pop      rbx
        ret
SFMT_Generate endp
...
SFMTBRandom PROC                         ; generate random bits
...
; y = ((uint32_t*)state)[ix++];
        mov     eax, dword ptr [par1+rdx].CRandomSFMTA.STATE
        add     edx, 4
...
SFMTRandom PROC                   ; generate random float with 52 bits resolution
        ; Align Pthis by 16. Will overlap part of Fill1 if Pthis unaligned        
        and     par1, -16
...
L1:     ; read 64 random bits
        movq    xmm0, qword ptr [par1+rdx].CRandomSFMTA.STATE
        add     edx, 8
        mov     [par1].CRandomSFMTA.IX, edx
        ...
        ; add mother bits
        movq    xmm1, qword ptr [par1].CRandomSFMTA.M0    ; Mother random number MC and M0
...
SFMTRandomL PROC                   ; generate random float with 63 bits resolution
        ; Align Pthis by 16.
        and     par1, -16
...
ConvertToFloat:
        ...
        movq    qword ptr [rsp], xmm0       ; store mantissa
        mov     dword  ptr [rsp+8], 3FFFH   ; exponent
        fld     tbyte ptr [rsp]             ; load long double
        fsub    [par1].CRandomSFMTA.one     ; subtract 1.0 to get 0.0 <= x < 1.0
...
M20:    ; do { // Rejection loop
        ...
        mul     [par1].CRandomSFMTA.LASTINTERVAL
...
; par2d = IncludeMother
        ...
        mov     par2d, type CRandomSFMTA ; ThisSize
...
SFMTgenRandom PROC                     ; generate random float with 52 bits resolution
        lea     par1, [SFMTInstance]   ; Get address of SFMTInstance into par1
...
;  extern "C" double SFMTgenRandom();
SFMTgenRandomL PROC                     ; generate random float with 63 bits resolution
        lea     par1, [SFMTInstance]    ; Get address of SFMTInstance into par1
...
SFMTgenBRandom PROC                     ; generate random float with 32 bits resolution
        lea     par1, [SFMTInstance]    ; Get address of SFMTInstance into par1
